(define (error x y) (display "Error: ") (display x) (display ": ") (display y)(newline))
;;; File:  regsim.scm
        ...
        (define (execute)
          (let ((insts (get-contents pc)))
            (if (null? insts)
                'done
                (begin
                  (cond ((equal? breakpoint '())
                         ((instruction-execution-proc (car insts)))
                         (execute))
                        (else
                         (let*((break (car breakpoint)))
                           (cond ((equal? break (car insts))
                                  (set! breakpoint (remove break breakpoint))
                                  (display "Execution Paused....")
                                  (display "Instruction: ")(display break)(newline))
                                 (else
                                  ((instruction-execution-proc (car insts)))
                                  (execute))))))))))  
        (define (set-breakpoint label n)
          (let*((instruction (get-inst (lookup-label labels label) n))
                (bp (create-breakpoint label instruction n)))
            (if(not(equal? instruction #f))
               (set! breakpoint (cons (breakpoint-inst bp) breakpoint)))))
        (define (cancel-breakpoint label n)
          (let*((instruction (get-inst (lookup-label labels label) n)))
            (if (is-bp? instruction breakpoint)
                (set! breakpoint (remove instruction breakpoint))
                (error "breakpoint is not defined at this instruction" instruction))))
        (define (dispatch message)
          (cond ((eq? message 'start)
                ...
                ((eq? message 'add-labels)
                 (lambda (new-labels) (set! labels new-labels)))
                ((eq? message 'set-breakpoint) (lambda (l n) (set-breakpoint l n)))
                ((eq? message 'proceed-machine) execute)
                ((eq? message 'cancel-all-breakpoints)
                 (lambda () (set! breakpoint '())))
                ((eq? message 'cancel-breakpoint) (lambda (l n) (cancel-breakpoint l n)))
...
;-------- Dispatch Helpers ----------      
;------------------------------------
(define (get-inst instructions n)
  (define (get-inst-iter inst num)
  (cond((equal? instructions '()) #f)
       ((equal? num n) (car inst))
       (else
        (get-inst-iter (cdr inst) (+ num 1)))))
  (get-inst-iter instructions 1))
...
(define (is-bp? instruction breakpoints)
   (cond((equal? breakpoints '()) #f)
        ((equal? instruction (car breakpoints)) #t)
        (else
         (is-bp? instruction (cdr breakpoints)))))

;---------------------------------------
;----- Working with Breakpoints --------
;---------------------------------------

(define (create-breakpoint label inst n) (list label inst n))
;breakpoint instruction selector
(define (breakpoint-inst breakpoint) (cadr breakpoint)) ;the idea is if we can match this instruction then I will stop execution
...
(define (assemble controller-text machine)
  (extract-labels controller-text
                  (lambda (insts labels)
                    ...
                    ((machine 'add-labels) labels) ;adds a list of labels to the machine per pdf 
                    insts)))

(define (extract-labels text receive)
  ;(display "texts:")(newline)
  ;(display text)(newline)
  ;(display "recieve:")(newline)
      ...
      (extract-labels
       ...
       (lambda (insts labels)
         ...
         (let ((next-inst (car text)))
           ;(display "next-insts: ") (display next-inst)(newline)
           ;(display "labels") (display labels) (newline) 
           (cond((assoc next-inst labels)
                 (error "***This label already exists***" next-inst))
                (else
                 (if (symbol? next-inst)
...
(define (make-label-entry label-name insts)
  ;(display "THIS IS THE LABEL: ") (display (cons label-name insts))(newline)
  (cons label-name insts))

(define (lookup-label labels label-name) ;returns instruction sequence attached to labels 
  ;(display "label-name: ")(display label-name)(newline)
...
(define (tagged-list? exp tag)
  (if (pair? exp)
      ...
      #f))
;-----------------------------------
;--- factorial iter machine code ---(display (cons label-name insts)
;-----------------------------------
#|
(define factorial-iter
  (make-machine
   '(product counter n temp)
   `((*, *) (=, =) (+ , +))
   '(
     start
     ...
     (branch (label end))
     (assign temp (op *) (reg counter) (reg product))
     (assign product (op +) (reg product) (reg temp))
     (assign counter (op +) (reg counter) (const 1))
     (goto (label loop))
     end
     )))

;(set-register-contents! factorial-iter 'n 3)
;(start factorial-iter)
;(get-register-contents factorial-iter 'product)

;----------------------------
;----- expt recursion -------
;----------------------------

(define expt-recursion
  (make-machine
   '(b n cont val)
   (list(list '* *)(list '= =)(list '- -))
   '(start
     ...
     (branch (label basecase))
     (save cont)
     (save n)
     (save b)
     (assign b (reg b))
     (assign n (op -) (reg n) (const 1))
     (assign cont (label next))
...
(set-register-contents! expt-recursion 'b 5)
(set-register-contents! expt-recursion 'n 3)
(start expt-recursion)
(get-register-contents expt-recursion 'val)
|#
;----------------------------
;------ expt iter -----------
;----------------------------
...
(define expt-iter
  (make-machine
   ...
   '(start
     ...
     finish)))
|#
;(set-register-contents! expt-iter 'n 3)
;(set-register-contents! expt-iter 'b 5)
;(start expt-iter)
...
;(set-breakpoint gcd-mac 'start 3)
;(cancel-breakpoint gcd-mac 'start 3)
;(set-register-contents! gcd-mac 'a 206)
;(set-register-contents! gcd-mac 'b 40)
;(start gcd-mac)
;(get-register-contents gcd-mac 'a)
;(proceed-machine gcd-mac)
;(get-register-contents gcd-mac 'a)
